                                      ...
                                      vector<Eigen::VectorXf> & labels)
{
    ...
    //store cmaera pose from input in to cv::Mat
    ...
    // Caluculate camera depth in wolrd coordinate based on camera pose and deep image feature 
    ...
    //pick a random entry from depth_img 
	...
	//store the slected points features 
        ...
        cv::Vec3d wld_pt = world_coordinate.at<cv::Vec3d>(y, x);// world coordinate based on depth image 
        Eigen::VectorXf label = Eigen::Vector3f(wld_pt[0], wld_pt[1], wld_pt[2]);
        labels.push_back(label); // label features with world coordinate 
    }
                                        ...
                                        const bool verbose)
{
    ...
    for (int i =0; i<features.size(); i++) {
        ...
        locations.push_back(cv::Point2d(x, y)); //location in the image (x,y)
    }
    ...
    //store selected pts x_descriptor
    for (int i = 0; i<features.size(); i++) {
